// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.

// <auto-generated/>

#nullable disable

using System.Collections.Generic;

namespace Microsoft.PowerBI.Api.Models
{
    /// <summary> The OData response wrapper for a list of Power BI gateways. </summary>
    public partial class Gateways
    {
        /// <summary> Initializes a new instance of <see cref="Gateways"/>. </summary>
        internal Gateways()
        {
            Value = new ChangeTrackingList<Gateway>();
        }

        /// <summary> Initializes a new instance of <see cref="Gateways"/>. </summary>
        /// <param name="odataContext"></param>
        /// <param name="value"> The list of gateways. </param>
        internal Gateways(string odataContext, IReadOnlyList<Gateway> value)
        {
            OdataContext = odataContext;
            Value = value;
        }

        /// <summary> Gets the odata context. </summary>
        public string OdataContext { get; }
        /// <summary> The list of gateways. </summary>
        public IReadOnlyList<Gateway> Value { get; }
    }
}
